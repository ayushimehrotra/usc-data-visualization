...
AU = ["Upper Brow Raiser", "Lower Brow Raiser", "Brow Lowerer", "Upper Lid Raiser", "Cheek Raiser", "Lid Tightener",
      "Nose Wrinkler",
...
def visual(y_pred):
    ...
    while cap.isOpened():
        ...
        if y_pred[count] != al_arr[count]:
            wrong = 1
            cv2.putText(frame, 'Model Predicted WRONG', (int(width - 200), int(height / 5 - 5)), font, 0.5, (0, 0, 255),
                        2)
